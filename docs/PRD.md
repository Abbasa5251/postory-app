...
- [~] **VERIFY the Zernio wire shapes (§3)** — *partially resolved 2026-07-18 against the Zernio **OpenAPI spec v1.0.4** (`docs.zernio.com/api/openapi`):* account **display fields confirmed** (the `SocialAccount` schema: avatar = `profilePicture` (nullable), handle = `username`/`displayName`; list wrapper = `{ accounts, hasAnalyticsAccess }`) and the **disconnect verb/path confirmed** (`DELETE /v1/accounts/{accountId}`) — both now tightened in `src/server/services/zernio/`. **Still open:** the `/accounts/health` **response body** shape. The spec actually surfaces health directly on each list item (`isActive`, `needsReconnection`) plus a `status=connected|disconnected` query filter — so the follow-up is to **retire the guessed `/accounts/health` call and derive `healthToStatus` from those confirmed list fields** instead. `ZERNIO_WEBHOOK_SECRET` + all Zernio webhooks remain **F3**.
...
- [ ] **Commit + PR the B3 post-merge fixes** (working tree, 2026-07-18): the first-connect persistence fix (`reconcile.ts` — §13 auth/DAL-adjacent, needs human review), the avatar `profilePicture` fix (`schemas.ts` + tests), and the npm/kysely reinstall. Ship the reconcile fix and the schema fix as separate small PRs (one concern each, §12). Then **verify in a browser**: connect an Instagram account → it appears on the Accounts page with its handle **and profile image** → Refresh keeps it.
- [ ] **Retire the guessed `/accounts/health` call (§3):** the Zernio OpenAPI v1.0.4 `SocialAccount` list item carries `isActive` + `needsReconnection`, and `GET /accounts` takes a `status=connected|disconnected` filter — derive `healthToStatus` from those confirmed fields instead of the separate best-effort `/accounts/health` endpoint whose response body is still unverified. Folds into the B3 status path (§13 — status is a §8 domain invariant).
