...
- [ ] B1. Brand CRUD (create + update) — Zernio profile provisioning moved to B3 (ADR-009 amended: lazy, on first account placement) **(P0)** — *B1.1 merged, B1.2 open (checkbox flips to [x] when #21 merges). Grilled → spec #17 → two tracer-bullet tickets. **B1.1 Create** merged as PR #20 (`feat/b1.1-brand-create`, closes #18). **B1.2 Edit** open in PR #21 (`feat/b1.2-brand-edit`, closes #19; now targeting master after #20's merge). Both §13 DAL/action hotspots — human review before merge. Scope: name + timezone only (logo/colors/`requires_client_approval` deferred to their consuming epics; voice profile + client-contact are B2); slug auto-derived + immutable, routing by id; timezone IANA-validated server-side; entitlement/plan cap deferred to B4. No Zernio call, no migration. See Epic B progress log.*
...
- [ ] `useActionForm` (`src/hooks/use-action-form.ts`) is now the shared form↔`ActionResult` glue — the next form (B2 voice profile / client contact) should use it rather than re-rolling submit state; supersedes the `toFormAction`/`useActionState` note in `src/server/actions/README.md` (reconcile the README when B2 lands).
