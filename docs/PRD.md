...
| **Last updated** | 2026-07-15 — A2 + A3 merged to `master` (PR #2: better-auth core + organizations, review fixes, migration squash, stylelint); see Epic A progress log |
...
- [ ] A1. Repo: Next 16 + TS strict + Tailwind 4 + shadcn/ui; ESLint/Prettier; Vitest + Playwright; GitHub Actions CI **(P0)** — *partial: repo/TS/Tailwind/shadcn done; stylelint with Tailwind-v4-aware config + `lint:css` script + VS Code workspace settings added in PR #2. Vitest, Playwright, CI, Prettier, ESLint boundary rules pending*
...
- [ ] `auth.ts` and `permissions.ts` intentionally omit `import 'server-only'` (the better-auth CLI rejects it during schema generation) — re-check on CLI upgrades whether the restriction is lifted. Note: the `auth:generate` npm script was dropped in the migration-squash commit; next schema regen needs it re-added or the CLI run ad-hoc (`SKIP_ENV_VALIDATION=1 auth generate --config src/server/auth/auth.ts --output src/db/schemas/auth.ts --y`)
