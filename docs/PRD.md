...
- [ ] B1. Brand CRUD (create + update) — Zernio profile provisioning moved to B3 (ADR-009 amended: lazy, on first account placement) **(P0)** — *in progress. Grilled → spec #17 → two tracer-bullet tickets. **B1.1 Create** done in PR #20 (`feat/b1.1-brand-create`, closes #18) — pending review (§13 DAL/action hotspots). **B1.2 Edit** (#19, blocked_by #18 via native GitHub dependency) not started. Scope: name + timezone only (logo/colors/`requires_client_approval` deferred to their consuming epics; voice profile + client-contact are B2); slug auto-derived + immutable, routing by id; timezone IANA-validated server-side; entitlement/plan cap deferred to B4. No Zernio call, no migration. See Epic B progress log.*
...
#### Epic B progress log & carry-over (added 2026-07-17, B1.1)
...
**Carry-over — do later, don't forget:**
- [ ] **B1.2 Edit** (#19) — the real permission is **`brand:update`** (per `permissions.ts` `statement`), NOT the ticket's loose `brand:edit`. Reuses B1.1's slug/validation foundation + `db-mock` extension; adds `updateBrandSchema`, `updateBrand` DAL (org+id-scoped, `brand.updated` audit), the update action, and a `/brands/[brandId]/settings` form + list link. Slug stays immutable on rename.
...
- [ ] Brand-scoped member narrowing for creators still pending B5 (`getAuthCtx` returns `brandIds: "all"`); `listBrands` already honors `ctx.brandIds` once populated.
- [ ] B1.1 UI not driven in a browser (role-gated button, dialog flow) — same deferred seeded-DB/session infra as the A8 live tests + Playwright nightly; the vertical slice is proven via the seam tests + build.
