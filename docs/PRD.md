...
| **Last updated** | 2026-07-17 — A8 merged as PR #9 (`test/a8-authz-suite`): exhaustive §7 role×permission matrix + mock-level DAL tenancy proof on shared fixtures + convention doc; live two-org DB tests and portal-token column deferred as tracked `it.todo`; one review round applied pre-merge (owner-only `organization:delete` assertions, realistic `creator` ctx default, prettier + doc wording); A5 merged (PR #6); A4 done (PR #4); A1 merged (PR #3); A2 + A3 merged in PR #2; §13 conventions marked done (AGENTS.md); see Epic A progress log |
