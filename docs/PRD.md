...
- [ ] **Verify PR #34 (auth-gate fix) in a real browser session** before/after merge: (a) member's null `active_organization_id` (DB row + `ba:*` Redis key cleared) → `/dashboard` recovers to the org, not onboarding; (b) switcher no longer offers "personal account"; (c) a zero-org user still routes to `/onboarding`. Unit-level covered; the live session flow is the deferred seeded-DB/session infra gap (shared with A8/Playwright).
- [ ] **VERIFY the Zernio wire shapes (§3) before B3 is exercised against production**: account display fields (handle/avatar), the account-health response shape + path, and the disconnect verb/path are parsed defensively in `src/server/services/zernio/` (see its README). Tighten the schemas + `healthToStatus` once confirmed. `ZERNIO_WEBHOOK_SECRET` + all Zernio webhooks remain **F3**.
