...
| **Last updated** | 2026-07-15 — A4 done (PRs #4 audit_log migration + A4 hardening PR: Upstash rate limits, Redis secondaryStorage, HIBP password check, login audit, cookie review, member-role rejection); A1 merged (PR #3); A2 + A3 merged in PR #2; §13 conventions marked done (AGENTS.md); see Epic A progress log |
...
- [ ] A5. Drizzle schema + DAL with mandatory org scoping; better-auth tables integrated **(P0)** — *better-auth tables integrated (CLI-generated `src/db/schemas/auth.ts` + migrations, relations merged); `AuthCtx`/`getAuthCtx()` exist in `src/server/auth/context.ts`. In review on `feat/a5-schema-dal`: all §4 domain tables + `credit_rates`, DAL foundation (org-scoped access, system ctx, mutation audit). Review hardening 2026-07-16: composite ownership FKs across the org/brand/post graph (incl. new `post_platforms.brand_id`), stage↔decider CHECK on approvals, credit_ledger append-only PG triggers (custom migration; a5 migration regenerated in place pre-merge) — all constraints probe-tested on a throwaway PG 18*
