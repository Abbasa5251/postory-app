...
- [ ] B3. Connect accounts via Zernio OAuth (6 launch platforms); all of a brand's accounts share its single lazily-provisioned Zernio profile (ADR-009 1:1 — no overflow profiles); connection health + reconnect prompts **(P0)** — *also owns primary-profile provisioning (`POST /api/v1/profiles`) — moved here from B1 by the ADR-009 lazy amendment*
