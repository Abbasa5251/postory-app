...
// The tx handle the transaction() callback receives — the same spies as the
// mocked db, so captureUpdate/captureDelete/captureInserts observe calls made
// through `tx` inside applyBrandUpdate/syncSocialAccount/deleteSocialAccountById.
const txMock = { select, insert, update, delete: del };
...
describe("syncSocialAccount — org scoping is structurally present", () => {
  it("updates by (org_id = ctx.orgId, zernio_account_id) and audits", async () => {
    ...
    captureInserts(insert); // audit insert inside the tx
    makeTransaction(transaction, txMock);
