 ...
 * so the update and its audit run in one atomic db.transaction.
 */
async function applyBrandUpdate(
  ...
  return db.transaction(async (tx) => {
    ...
    // 0 rows: a genuine race (deleted between the scoped fetch and here). We
    // throw before the audit write, so the transaction rolls back cleanly —
    // no orphan audit row (an improvement over the old batch's residual).
    if (!updated) throw new NotFoundError("brand", brandId);
