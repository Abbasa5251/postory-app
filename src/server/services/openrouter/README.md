...
AI inference goes through OpenRouter). Text captions now (C2); the dedicated
Image API lands here for D2. Preserves a direct-provider escape hatch: nothing
outside this module knows the transport.

## Wire layer (AGENTS.md §3)

Verified against the installed packages:

- `ai@7.0.31` — `streamText({ model, system, prompt, temperature, maxOutputTokens, abortSignal })`
...
- `@openrouter/ai-sdk-provider@3.0.0` — `createOpenRouter({ apiKey, appName, appUrl })`
  → provider; `provider.chat(modelId)` is the language model. `apiKey` from
  `env.OPENROUTER_API_KEY` (lazy — unset only fails at first real call, like the
  Zernio client / getRedis).

Text is OpenAI-compatible chat completions. Model ids come from the
`credit_rates` config table via `dal/credits.ts` (`getActiveRate`) — never
hardcoded here (ADR-012).
