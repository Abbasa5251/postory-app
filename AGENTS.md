...
- **Drizzle / migrations (Neon Postgres 18):** A child table's **composite tenancy FK** `(org_id, id) → parent(org_id, id)` (the §6.4 belt-and-suspenders pattern) requires the **parent to declare a matching `unique(...).on(orgId, id)`** — otherwise `npm run db:generate` emits SQL that is *silently* invalid and fails **only at `db:migrate` apply time** (`no unique constraint matching given keys`); typecheck + lint + build + unit tests all stay green (they never apply the migration). So: verify the parent carries the `*_org_id_id_key` unique before adding the child FK, and **actually run `db:migrate` against the local docker Postgres as part of review** — CI's `build` does not apply migrations. Snapshots live inside each migration folder (no shared `meta/`); regenerate with `db:generate` after schema edits, never hand-edit an applied migration. For an **atomic create that needs child rows + an audit row in one `db.batch`**, generate the row id app-side (`crypto.randomUUID()`) so sibling statements can reference it — a DB-generated `uuidv7()` id can't be (see `dal/audit.ts` Case A/B and `dal/comments.ts` `createComment`).
- **Custom popups (mention typeahead, etc.):** an absolutely-positioned dropdown inside a `<Card>`, `<Dialog>`, or any scroll container gets **clipped by that ancestor's `overflow`**. Render it in a **portal** (`createPortal(..., document.body)`) with `position: fixed`, tracking the anchor's `getBoundingClientRect()` and realigning on `scroll`/`resize` (capture). Reference: `src/components/features/comments/mention-textarea.tsx`. (The Base UI `Popover`/`Combobox` primitives already portal — prefer them for new work.)
